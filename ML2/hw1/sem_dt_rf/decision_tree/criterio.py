...
class GiniCriterion(Criterion):
    ...
    def get_predict_val(self, target: np.ndarray) -> np.ndarray:
        #Считает число обьектов каждого класса и деля их на шейп получаем вектор ?вероятностей, он единичен по первой норме 
...
class MSECriterion(Criterion):
    def get_predict_val(self, target: np.ndarray) -> float:
        if target.size == 0:
            return 0 
    ...
    def score(self, target):
        ...
        return np.sum((self.get_predict_val(target) - target) ** 2) / target.shape[0]
