...
from sem_dt_rf.decision_tree.criterio import GiniCriterion, EntropyCriterion, MSECriterion
...
class TestMSECriterion(unittest.TestCase):
    def test_entropy_get_predict_val(self):
        target = np.array([1, 1, 2, 4, 2, 2, 0, 1, 0, 4])
        y_true = np.array([0.2, 0.3, 0.3, 0, 0.2])
        y_pred = EntropyCriterion(n_classes=5).get_predict_val(target)
...
class TestMSECriterion(unittest.TestCase):
    def test_mse_get_predict_val(self):
        target = np.array([1, 1, 2, 4, 2, 2, 0, 1, 0, 4])
        y_true = np.mean(target)  # Среднее значение
        y_pred = MSECriterion().get_predict_val(target)
        assert np.isclose(y_pred, y_true)

    def test_mse_score(self):
        target = np.array([1, 1, 2, 4, 2, 2, 0, 1, 0, 4])
        mse_true = np.var(target)  # Дисперсия = MSE для средних значений
        mse_pred = MSECriterion().score(target)
        assert np.isclose(mse_pred, mse_true)

    def test_mse_get_best_split(self):
        n = 100
        x = np.arange(n)
        num_zeros = n // 2
        y = np.r_[np.ones(num_zeros), np.zeros(n - num_zeros)].astype(int)
        ...
        threshold, q_best = criterion.get_best_split(x, y)

        assert np.isclose(threshold, 49.5)
        assert q_best > 0  # MSE должно уменьшаться после разбиения

    def test_mse_get_best_split_v2(self):
        x = np.array([0.1, 0.4, 0.2, 0.3, 0.5, 0])
        y = np.array([0, 1, 0, 1, 1, 0])

        criterion = MSECriterion()
        threshold, _ = criterion.get_best_split(x, y)
